...
| 許可の自動化 (Skip) | 設定の「**許可を自動で出す**」をオンにすると、ツール実行の許可をノッチが自動で許可する（確認なし）。**AIからの質問と計画の実行確認は対象外**で必ず人間が決める。既定はオフ |
| 質問 (Ask) | AIからの質問（AskUserQuestion）は**許可とは別扱い**。質問文と選択肢を表示し、「**質問に答える**」ボタンでそのAIのウィンドウへ移動する（回答は画面側で行う） |
| 計画の承認 (Plan) | プランモードの「この計画で進めてよいか」（ExitPlanMode）も**許可とは別扱い**。計画の冒頭だけを表示し、「**計画を確認する**」ボタンでそのAIのウィンドウへ移動する（全文を読んで承認するのは画面側） |
...
| いまの場所 | つないでいるWi-Fiから居場所を判定（スタバ / マック / コメダ / シアトルズ / 自宅など）。設定画面とメニューに表示し、`GET /place` で外からも引ける。**macOSの位置情報許可が必要**（下記） |
...
**AIからの質問（AskUserQuestion）と、プランモードの「この計画で進めてよいか」（ExitPlanMode）はこの対象外**。どちらも自動では答えず、「質問に答える」「計画を確認する」ボタンからその画面へ移動して人間が決める。「作業の許可は全部OK、判断が要るところだけ自分で読む」という使い方のための設定。

- 既定はオフ（＝すべての許可を人間が出す）。オフにすれば元の動作に戻る
- メニューバーのClawdアイコン右クリックからも切り替えられる（オンにするときだけ確認が出る）
- 削除やコマンド実行も含めて**ツール実行の許可要求はそのまま許可する**（ツールごとの絞り込みはしない。例外は上の2つだけ）
...
つないでいるWi-Fiの名前（SSID）から「いまどこにいるか」を出す。設定画面の「いまの場所」と、メニューバーアイコンの右クリックメニューに表示され、`curl http://127.0.0.1:43110/place` で外からも引ける。

**macOS 14以降、現在のSSIDは位置情報の許可がないプロセスには伏せられる**（CLIで `ipconfig getsummary en0` を叩くと `SSID : <redacted>` が返るのと同じ制限）。許可がないとスタバも自宅も区別がつかず、全部同じに見える。

```
システム設定 > プライバシーとセキュリティ > 位置情報サービス
  → 一番上のスイッチをオン → 一覧の「AI Notch」をオン
```

- 位置情報サービス自体がオフだと**一覧にアプリが出てこない**。先に大元のスイッチを入れる
- 許可を使うのは**SSIDの秘匿を外すためだけ**で、緯度経度は取得も記録もしない
- スタバ・マック・コメダ・シアトルズ・タリーズ・ドトールなどのチェーンは、SSIDの部分一致で名前を出す（`WiFiPlace.rules`）
- 自宅・実家など組み込みルールに無い場所は、設定画面の入力欄に名前を入れて保存すると、そのSSIDに紐づいて覚える（空で保存すると消える）
- 名前が付かないWi-FiはSSIDをそのまま表示する（「不明」で潰さない）

```bash
curl http://127.0.0.1:43110/place
# {"place":"スタバ","ssid":"at_STARBUCKS_Wi2","recognized":true,"location_authorized":true, ...}
```
...
curl http://127.0.0.1:43110/place     # いまの場所（Wi-Fi名・位置情報の許可状態）
```
