...
| `hooks/notch-hook.sh` | Codex hooks から呼ばれる転送スクリプト |
| `hooks/notch_post.py` | POST共通処理（全スクリプトが使用） |
| `hooks/install-hooks.sh` | `~/.Codex/settings.json` への登録（バックアップ+冪等） |
...
6. `~/.codex/config.toml` の notify は Codex Cowork が使用中。**上書き禁止**。
7. このフォルダは独立gitリポジトリ。コミットはこの中で行う。

## イベントAPI（POST /event）

Codex hooks形式（`hook_event_name`: SessionStart / UserPromptSubmit / PreToolUse / PostToolUse / PermissionRequest / Notification / Stop / SessionEnd）と汎用形式（`event`: start / status / done / error / remove）の両方を受け付ける。ターミナル特定用に `tty` / `term_program` / `iterm_session_id` / `bundle_id` を付加する。
